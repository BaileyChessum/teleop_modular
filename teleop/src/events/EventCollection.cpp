...
#include "teleop/events/ButtonEvent.hpp"
...
EventCollection::EventCollection(std::weak_ptr<EventListenerQueue> listener_queue, InputManager& inputs)
  ...
  , factory_{ { "/down",
                [&inputs](const std::string& name, const std::weak_ptr<teleop::EventListenerQueue>& queue) {
                  return std::make_shared<teleop::internal::ButtonEvent>(name, queue, true, inputs.get_buttons());
                } },
              { "/up", [&inputs](const std::string& name, const std::weak_ptr<teleop::EventListenerQueue>& queue) {
                 return std::make_shared<teleop::internal::ButtonEvent>(name, queue, false, inputs.get_buttons());
               } } }
{
}
...
Event::SharedPtr EventCollection::create_event_for_name(const std::string& name)
{
  ...
  auto it =
      std::find_if(factory_.begin(), factory_.end(), [&](const auto& item) { return ends_with(name, item.first); });
