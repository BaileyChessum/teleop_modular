...
![Example mapping of inputs in a diagram](./teleop_modular/docs/images/big_teleop_input_mapping_example_light.drawio.svg)
...
Control modes are plugins that consume named inputs and publish control messages to some downstream system (e.g. `geometry_msgs/Twist` for a mobile base, or joint velocity commands for an arm).

Each control mode has its own node and is configured via ROS2 parameters. A `teleop_node` can run multiple control modes simultaneously, with any combination active at a time.
