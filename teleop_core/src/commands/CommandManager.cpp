...
void CommandManager::create_command(const std::string & name, EventCollection & events)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      logger, "Failed to find command plugin \"%s\" for command \"%s\"!\nwhat(): %s", type.c_str(),
      name.c_str(), ex.what());
    return;
  }
  ...
  command_class->initialize(
    context_, name, stored_events, node_->get_node_logging_interface(),
...
void CommandManager::configure(EventCollection & events)
{
  ...
  node_->declare_parameter(
    command_definitions_descriptor.name, rclcpp::ParameterValue(std::vector<std::string>()),
  ...
  const auto command_names = command_definitions_param.get_type() ==
...
bool CommandManager::get_type_for_command(
  ...
  std::vector<std::string> & invocation_event_names) const
{
  ...
  type_descriptor.description = "The name of the plugin to load as the class for the \"" + name +
    "\" command";
  node_->declare_parameter(
    type_descriptor.name, rclcpp::ParameterType::PARAMETER_STRING,
    type_descriptor);
  ...
  const auto on_any = utils::get_parameter<std::vector<std::string>>(
    parameters, "commands." + name + ".on_any",
  ...
  const auto on = utils::get_parameter<std::string>(
    parameters, "commands." + name + ".on",
